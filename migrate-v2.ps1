...
.DESCRIPTION
    The package folder must contain:
      - migrate-v2.ps1   (this script)
      - config.json   (settings)
      - SophosSetup.exe  (or whatever filename is set in config.json)
	  - CSUninstallTool.exe (downloaded from CrowdStrike console)
    ...
    Workflow:
      ...
      5. Uninstall CrowdStrike Falcon (GUID looked up from registry; flags from config.json) using CSUninstallTool.
...
if (-not $falconEntry) {
    ...
    Log "Running: $($cfg.CrowdStrikeCSUninstallTool)"

    try {
        $proc = Start-Process -FilePath $cfg.CrowdStrikeCSUninstallTool -ArgumentList $cfg.CrowdStrikeCSUninstallToolArgs -WindowStyle Hidden
        # 0 = success, 3010 = success with pending reboot
        if ($proc.ExitCode -notin @(0, 3010)) {
            Log "Tool exited with code $($proc.ExitCode)." "ERROR"
            Stop-Transcript | Out-Null
            exit 1
        }
        Log "Uninstall process completed (exit code $($proc.ExitCode))."
    } catch {
        Log "Failed to run uninstall tool: $_" "ERROR"
        Stop-Transcript | Out-Null
        exit 1
    }
}
